"""Validate docs/addins/mail-triage/manifest.xml structure and content."""
...
MANIFEST = ADDIN_MANIFEST_PATHS["mail-triage"]
BASE_URL = ADDIN_BASE_URLS["mail-triage"]
...
def test_extension_name_in_taskpane_js():
    js = (MANIFEST.parent / "taskpane.js").read_text(encoding="utf-8")
    assert "EXTENSION_NAME" in js
    assert '"net.cprima.rpapub.CPMForge.M365.triage"' in js


def test_client_id_is_set():
    js = (MANIFEST.parent / "taskpane.js").read_text(encoding="utf-8")
    assert "REPLACE_WITH_YOUR_CLIENT_ID" not in js, "CLIENT_ID placeholder not replaced"
